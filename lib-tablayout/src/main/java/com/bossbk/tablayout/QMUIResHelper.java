...
import com.bossbk.tablayout.QMUIHelper;
...
public class QMUIResHelper {
    ...
    public static Drawable getAttrDrawable(Context context, TypedArray typedArray, int index){
        ...
        if(value != null){
            if(value.type != TypedValue.TYPE_ATTRIBUTE && value.resourceId != 0){
                return QMUIHelper.getVectorDrawable(context, value.resourceId);
            }
        }
        return null;
    }
    ...
    public static int getAttrDimen(Context context, int attrRes){
        TypedValue typedValue = new TypedValue();
        context.getTheme().resolveAttribute(attrRes, typedValue, true);
        return TypedValue.complexToDimensionPixelSize(typedValue.data, QMUIHelper.getDisplayMetrics(context));
    }
